...
const ACTION_STYLES: Record<string, string> = {
  ...
  assignment_cancelled: "bg-(--text-muted)/20 text-(--text-muted)",
  student_verified: "bg-green-500/20 text-green-400",
  student_deactivated: "bg-red-500/20 text-red-400",
  upload_rejected: "bg-red-500/20 text-red-400",
  roster_changed: "bg-purple-500/20 text-purple-400",
  semester_rollover: "bg-amber-500/20 text-amber-400",
};
  ...
  return (
    <div className="flex flex-col gap-6 p-6 max-w-6xl">
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-surface">
        <div className="grid grid-cols-[160px_1fr_1fr_140px] gap-3 border-b border-border bg-elevated px-4 py-2.5">
          <span className="text-xs font-semibold uppercase tracking-wider text-(--text-muted)">Action</span>
          <span className="text-xs font-semibold uppercase tracking-wider text-(--text-muted)">Performed By</span>
          <span className="text-xs font-semibold uppercase tracking-wider text-(--text-muted)">Details</span>
          <span className="text-xs font-semibold uppercase tracking-wider text-(--text-muted)">Time</span>
        </div>
                ...
                <span>
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${
                      ACTION_STYLES[log.action_type] ?? "bg-elevated text-(--text-muted)"
                    }`}
                  >
          ...
          <div className="flex gap-2">
            {page > 1 && (
              <Link
                ...
                className="rounded-lg bg-elevated px-3 py-1.5 text-xs hover:bg-border transition-colors"
              >
                ← Prev
              </Link>
            )}
